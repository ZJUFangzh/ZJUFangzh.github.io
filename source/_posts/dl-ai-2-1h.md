...
使用`np.sum(np.square(Wl))`来计算$\sum\limits_k\sum\limits_j W_{k,j}^{[l]2}$
...
1. 每一层的$d^{[l]}$对应每一层的$a^{[l]}$,因为有m个样本，所以就有$D^{[1]} = [d^{[1](1)} d^{[1](2)} ... d^{[1](m)}] $of the same dimension as $A^{[1]}$.使用np.random.rand(n,m)
...
反向传播的时候，让之前的删除的neurons依旧归0，然后也要除以keepprob，因为`dA = np.dot(W.T, dZ)`，并没有重复除以过系数。
